...
export async function scanForDisc(timeoutMs = 20000) {
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(new Error('Timed out scanning for DiscDawg. Make sure Pico is powered and advertising.'));
    }, timeoutMs);
...
async function readStatus(device: any) {
  const result = await device.readCharacteristicForService(SERVICE_UUID, STATUS_UUID);
  return decodeBleUtf8(result.value);
}

async function readData(device: any) {
  const result = await device.readCharacteristicForService(SERVICE_UUID, DATA_UUID);
  return decodeBleUtf8(result.value);
}
...
export async function downloadFlightFromDisc(
  ...
  if (initialStatus.startsWith('IDLE')) {
    throw new Error('Disc has no flight data yet. Throw first, then scan.');
  }
...
export async function readRealtimeSample(device: any): Promise<RealtimeSample | null> {
  const result = await device.readCharacteristicForService(SERVICE_UUID, MONITOR_UUID);
